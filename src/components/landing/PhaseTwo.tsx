...
const PhaseTwo = () => {
  ...
  return <section className="section-padding bg-foreground/5">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex justify-center mb-10">
            <div className="inline-flex bg-background border border-border rounded-full p-1 shadow-lg">
              <button onClick={() => setHasDatabase(true)} className={`px-6 py-3 rounded-full text-sm font-bold tracking-wider transition-all duration-300 ${hasDatabase ? "bg-accent text-white shadow-md" : "text-muted-foreground hover:text-foreground"}`}>
                I HAVE A DATABASE
              </button>
              <button onClick={() => setHasDatabase(false)} className={`px-6 py-3 rounded-full text-sm font-bold tracking-wider transition-all duration-300 ${!hasDatabase ? "bg-accent text-white shadow-md" : "text-muted-foreground hover:text-foreground"}`}>
                I AM STARTING FRESH
              </button>
            </div>
          </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={hasDatabase ? "database" : "fresh"} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
            ...
            ease: "easeInOut"
              ...
              <div className="relative">
                ...
                <div className="hidden md:block absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-px">
                  ...
                  <motion.div className="absolute left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-accent shadow-[0_0_15px_hsl(var(--accent))]" animate={{
                  y: [0, 200, 0],
                  opacity: [0.5, 1, 0.5],
                  scale: [0.8, 1.2, 0.8]
                }} transition={{
                ...
                <div className="grid md:grid-cols-3 gap-8 mb-12">
                  {currentContent.pillars.map((pillar, index) => <motion.div key={pillar.title} initial={{
                  opacity: 0,
                  y: 30
                }} animate={{
                  opacity: 1,
          ...
          <div className="bg-background border border-border rounded-lg p-6 text-center">
            ...
            <a href="#contact-form" onClick={handleScrollToForm} className="inline-flex items-center text-accent hover:text-accent/80 font-medium transition-colors">
